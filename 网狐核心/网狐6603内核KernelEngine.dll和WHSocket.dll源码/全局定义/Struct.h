...
#define LEN_VERSION					16
#define LEN_KIND_NAME				32
//游戏列表
struct tagGameList
{
	TCHAR							szClientVersion[LEN_VERSION];		//客户端版本
	WORD							wKindID;							//类型标识
	WORD							wKindMark;							//
	TCHAR							szKindName[LEN_KIND_NAME];			//游戏名
	TCHAR							szModuleName[LEN_KIND_NAME];			//模块名
	WORD							wResVersion;						//
	WORD							wSortID;							//排序标识
	WORD							wTypeID;							//种类标识
};
...
//获取推广奖励列表
struct tagRecordSpreadInfo
{
	float		lScore;													//提成
	TCHAR		szTypeNote[LEN_REMARKS];								//奖励类型 32
	TCHAR		szCollectDate[LEN_DATETIME];							//日期 LEN_DATETIME = 20
	TCHAR		szNickName[LEN_NICKNAME];								//用户昵称 32
};

//公告列表
struct tagNotifyInfo
{

	TCHAR							szNickName[LEN_NICKNAME];			//用户昵称
	TCHAR							szServerName[LEN_SERVER];			//房间信息
	SCORE							lScore;								//用户分数
};
...
struct tagUserInfo
{
	...
	//用户省份
	TCHAR							szProvince[LEN_PROVINCE];				//用户省份
	DWORD							dwIsSaleMan;							//是否是业务员	//0---普通用户，1------业务员
	//是否指定赢家
	DWORD							dwIsWinUser;							//是否指定赢家
...
struct tagPersonalRoomOption
{
	...
	SCORE		lPersonalRoomTax;				//私人房税收比例
